...
- 가시의 비극: 서막
    - 기둥 8개와 4개의 장판을 먼저 보여준 후 가시 울타리 기술을 시전하며 순차 활성화
...
- 가시의 비극: 제 2막
    ...
    - 탱커1명, 힐러1명에게 다쟈 부여 후 어둠 설계 시전
    - 이후 각각 안전지대로 이동해 기둥과 `헤미테오스 파이가` 처리
- 가시의 비극: 2막 타임라인
    - 탱커1명, 힐러1명에게 다쟈 부여 후 어둠 설계 시전
    - 다쟈가 부여된 탱커와 힐러는 각각 11시, 5시로 이동
    - 파이가, 에어로가가 부여된 인원들은 중앙에 모여서 어둠 설계을 한곳에 유도
    - 첫 안전지대 확인 후 다음과 같이 이동<br>다쟈/파이가 탱커, 에어로가 딜러 : 북 or 서<br>다쟈/파이가 힐러, 딜러 : 남 or 동
    - 탱커/힐러 파이가 대상자는 알파벳 징 위로 이동하여 파이가(3인 쉐어) 처리
    - 다쟈 탱커/힐러는 각각 숫자 징 위로 이동하여 기둥(선더) 처리
    - **파이가 처리 후 중앙으로 살짝 이동하여 체력 회복을 받을 것**
    - 첫 안전 지대 기준 다쟈, 파이가 탱커/힐러, 에어로가 는 시계 방향으로 이동
    - 에어로가 대상자는 에어로가가 끊어지지 않도록 천천이 같이 이동해야 함 (돌진기 X)
    - 파이가 딜러 대상자는 숫자가 빠른 순서대로 앞번호가 시계방향, 뒷번호가 반시계 방향 이동<br>(예시 : D3, D4가 파이가 대상일 경우 D4가 반시계 방향으로 이동)
    - 마지막 파이가 처리 후 극한 충격 캐스팅이 완료되기 전에 중앙으로 모여서 체력 회복받을 것
- 가시의 비극: 제 3막
    - 8개의 기둥을 각각 4개씩 동시 활성화 하며 시전 완료 후 `무대신 발길질`을 시전함
    - 방식에 따라 다르며 첫 점프(`무대신 발길질`) 유도자 이동 시 보스 근처에 있지 않아야 함
    - 점프 후 보스에게 가장 가까운 3명에게 좁은 부채꼴 공격 `무대신 땅울림`를 시전하므로 처리조 이동
    - 기둥조는 처리가 완료되면 다음 기믹을 대비한다
- 가시의 비극: 3막 타임라인
    - 첫번째 줄이 연결된 기둥 방향으로 힐러/원딜이 약산개
    - 무대신 발길질 시전 시 MT는 첫번째 줄이 연결된 기둥 반대편으로 보스 유도
    - 보스 이동 후 힐러/원딜은 각자 기둥으로 이동<br>
    기둥으로 이동하는 타이밍에 전력 질주+견고한 마법/거리 유지 사용
    - 보스가 이동하는 것을 본 뒤 ST와 D1, D2는 보스 타겟 범위 안쪽으로 들어간 다음<br>
    보스 머리 방향 기준 12시 / 3시 / 9시 위치에 서서 땅울림 유도. (땅울림 유도 시 거리유지 사용할 것)<br>
    <b>※무대신 땅울림은 보스 기준 가장 가까운 대상 3인에게 유도됨※</b>
    - 기둥 처리 직후 후 기둥조는 바로 반대편 기둥으로 이동
    - 워터쟈가 발동하면서 파티원 전원 약한 넉백(중앙 기준 맵 절반 정도 넉백, 거리 유지/견고한 마법 사용 시 넉백 방지 가능)
    - MT가 두번째 무대신 발길질을 중앙으로 유도
    - 힐러/원딜은 두번째 기둥 처리, ST/D1/D2 땅울림 유도 진행
    - MT는 발길질 유도 이후 확실하게 빠져서 땅울림가 본진 및 본인에게 유도되지 않도록 주의할 것<br>
    땅울림 유도하는 조가 보스 원형 범위 안으로 확실하게 들어가서 유도할 경우 MT는 6시에서 칼끝딜 가능
- 무대신 발길질
    - 보스로부터 가장 먼 1명에게 점프하여 범위 넉백
- 무대신 땅울림
    - 보스로부터 가장 가까운 3명에게 좁은 부채꼴 공격
    - `피마법 피해 증가` 디버프가 달려 있는 경우 사망하므로 발길질을 유도한 대상자는 빠져야 함
- 심장 말뚝
...
- 가시의 비극: 제 4막
    ...
    - 빠르게 처리하지 않으면 이후 오는 `극한 충격`에 `피마법 피해 증가`를 달고 있게 되므로<br>전멸하게 됨
- 가시의 비극: 제 4막 타임라인
    - 하단의 이미지 참고
- 헤미테오스 워터가
    - 대상자 중심 큰 범위 공격
...
>
> [Xenosys의 공략법](#act3-xenosys)은 서로의 움직임을 줄이는 방식이나 근거리 직업군에게 미묘한 공백이 발생함<br>따라서 필요에 따라 조율하여 사용
## 가시의 비극: 제 4막

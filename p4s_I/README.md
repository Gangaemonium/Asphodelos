...
- 피의 호수
    ...
    - 피의 호수 II
    줄이 연결되지 않은 무대가 안전 지대 (`페리악토이`를 시전하며 모든 무대 활성화)
- 에테르 클라미스
    - 보스 버프 변경 (검 &gt; 방패 / 방패 &gt; 검)
- 강화된 벨로네: 가면
    - 무작위 4명에게 전염 디버프 부여, 두번째 피의 호수 대상자가 아니었던 조가 처리
- 반전된 클라미스
    - 무작위 4명에게 채갈 수 있는 줄이 연결됨, 첫번째 피의 호수 대상자가 아니었던 조가 줄을 인계하여 처리
- 강화된 벨로네: 가면 / 반전된 클라미스 <b>YPP식 타임라인</b>
    1) `피의 호수`에서 탱/힐조 또는 딜러조가 두번 다 줄 대상자인 경우
        - 무작위 4명에게 전염 디버프 부여 (`탱/힐` &gt; 12시, `딜러` &gt; 6시 집합)
        - 두번째 `피의 호수` 대상자가 아니었던 조가 전염 처리
        - 전염 처리 후 본진 앞에 모여서 줄을 인계한 다음 산개
    2) `피의 호수`에서 탱/힐 1회, 딜러 1회 줄 대상자가 된 경우
        - 무작위 4명에게 전염 디버프 부여 (`탱/힐` &gt; 12시, `딜러` &gt; 6시 집합)
        - 두번째 `피의 호수` 대상자가 아니었던 조가 전염 처리
...
- 강화된 벨로네: 가면 / 반전된 클라미스 <b>공대식 타임라인</b>
    1) `피의 호수`에서 탱/힐조 또는 딜러조가 두번 다 줄 대상자인 경우
        - 무작위 4명에게 전염 디버프 부여 (두번째 `피의 호수` 대상 조가 6시 집합)
        - 두번째 `피의 호수` 대상자가 아니었던 조가 전염 처리
        - 전염 처리 후 본진 앞에 모여서 줄을 인계한 다음 산개
    2) `피의 호수`에서 탱/힐 1회, 딜러 1회 줄 대상자가 된 경우
        - 무작위 4명에게 전염 디버프 부여 (두번째 `피의 호수` 대상 조가 6시 집합)
...
- 페리악토이
    - `피의 호수`에서 연결된 바닥 3개가 폭발하며, 줄이 연결되지 않은 바닥이 안전 지대
- 강화된 벨로네: 원형
